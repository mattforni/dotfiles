...
Delegate matching work to the user level agent roster in `~/.claude/agents/` proactively, without waiting for explicit invocation. When a task matches an agent's description, dispatch to it rather than doing the work inline. Multi subsystem recon fans out to explore instances in parallel. Claims and fresh changes get a skeptic pass. Test and build runs go to runner. Mechanical multi file sweeps go to migrator instances with non overlapping file ownership. Web research goes to researcher. Driving an open PR to merged (bot review polling, feedback triage, the merge itself) goes to lander in the background rather than foreground polling; lander bails to the main session on human reviews, CI failures, and merge conflicts. Cap concurrent migrator writers at three; read only agents can fan wider. Agents return summaries with pointers, never transcripts. This does not override the rule against unprompted exploration; it governs how requested work gets executed, not whether to start work.
