...
1. Read the emailed retro (`YYYY-Www Retro` from `claude@atelic.me`, the runner's first pass with the movement week, the takeout tally, and the outreach week already graded) instead of pulling; pull a quick Strava glance only when it is missing.
