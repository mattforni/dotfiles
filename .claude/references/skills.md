...
- Human gates come in three shapes; pick one deliberately. Proposal board with resume to execute (clerk: propose, Forni corrects, the same agent executes the corrected board because it holds the IDs), gate and yes (unemployment-claimer: stop at each named gate, act past it only when resumed with an explicit yes), and bail to main (lander: enumerate the conditions that end autonomy and return).
