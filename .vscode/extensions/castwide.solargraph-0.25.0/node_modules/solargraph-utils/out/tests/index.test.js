...
suite('solargraphCommand', () => {
    ...
    test('works with the default command path', (done) => {
        ...
        child.on('exit', () => {
            (0, chai_1.expect)(output).not.to.equal('');
            done();
        });
    });
    test('works with a custom command path', (done) => {
        let cmd = 'solargraph';
        if (((0, os_1.platform)().match(/darwin|linux/))) {
            cmd += '.rb';
        }
        else {
            cmd += '.bat';
...
suite('SocketProvider', () => {
    ...
    test('starts', (done) => {
        (0, chai_1.expect)(provider.isListening()).to.equal(false);
        provider.start().then(() => {
            (0, chai_1.expect)(provider.isListening()).to.equal(true);
        }).catch(() => {
            throw new Error('SocketProvider failed to start');
        }).then(done, done);
    });
    test('opens a port', () => {
        (0, chai_1.expect)(provider.port).to.be.above(0);
    });
    test('restarts', (done) => {
