...
const ourSelectionsUpdates_1 = require("./src/mode/ourSelectionsUpdates");
...
async function activate(context, handleLocal = true) {
    ...
    registerEventListener(context, vscode.window.onDidChangeTextEditorSelection, async (e) => {
        ...
        mh.selectionsChanged.ourSelectionsUpdates = (0, ourSelectionsUpdates_1.cleanupOurSelectionsUpdates)(mh.selectionsChanged.ourSelectionsUpdates);
        if (e.kind !== vscode.TextEditorSelectionChangeKind.Mouse) {
            const selectionsHash = (0, ourSelectionsUpdates_1.hashSelections)(e.selections);
            const idx = mh.selectionsChanged.ourSelectionsUpdates.findIndex((entry) => entry.selectionsHash === selectionsHash);
            if (idx > -1) {
                mh.selectionsChanged.ourSelectionsUpdates.splice(idx, 1);
                logger_1.Logger.trace(`Ignoring selection: ${selectionsHash}. ${mh.selectionsChanged.ourSelectionsUpdates.length} left`);
                return;
            }
