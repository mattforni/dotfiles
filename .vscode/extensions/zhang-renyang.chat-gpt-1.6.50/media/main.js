; (function () {
	...
	function getAskBtnWrapper(message) {
		const { languageId='javascript' } = message;
	...
	function highlightBlock(str, lang) {
		return `<pre class="code-block-wrapper"><div class="code-block-header flex justify-between items-center"><span>${lang}</span>${getCodeBtnWrapper()}</div><code class="hljs language-${lang} code-block-body">${str}</code></pre>`
	}
	...
	function copyCode(targetButton) {
		const value = targetButton?.parentElement?.parentElement?.parentElement?.lastElementChild?.textContent;
		if (!value) return;
		navigator.clipboard
			.writeText(value)
			.then(() => {
