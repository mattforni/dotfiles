...
**That comparison has a trap, which is why `GWS_FORCE_PROFILE` exists.** "Equal to the stamp means inherited" is a guess, and it guesses wrong in one specific case: overriding to the profile the shell is **already on**. That value is byte identical to an inherited one, so the shim discards it and re-resolves from the `.account` marker, landing you on a different account than you asked for, silently. Override to any *other* profile and it works. The failure is therefore conditional on which profile you name, so testing the recipe once proves nothing about the next call, and an agent shell can spend a long time reading the wrong mailbox with no signal at all. That happened on 2026-08-17: three profiles' worth of Gmail searches all returned zero, and every one had queried the same mailbox.

**So `GWS_FORCE_PROFILE=<name> gws ...` is the correct one shot override**, and the config dir form should be treated as legacy. Nothing else ever sets that variable, so it carries no ambiguity; the shim consumes it before exec so it cannot leak into a nested call, and an unknown profile exits 78 with a message rather than falling back to a guess.
