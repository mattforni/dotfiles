...
- **Triage has a definition, and it is three fields.** An issue counts as triaged only when it carries exactly one cognitive load label (🧠 Sharp / ⚖️ Medium / 🍃 Light), an hourly estimate (points are hours on the linear extended scale), and tagging that actually describes the work. Missing any one of the three makes it untriaged no matter what its state says, and untriaged is always a Decision, never a silent Keep. **Due dates were a fourth field until 2026-08-21 and are now retired; an issue still carrying one is drift to flag.**
